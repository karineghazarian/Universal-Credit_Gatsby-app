...
function Calculator(props)
{
  ...
  function calculate(e)
  {
    ...
    if (amount && percent && months && type)
    {
      ...
      if (type === "1")
      {
        let final = amount * (r * Math.pow(1 + r, months)) / (Math.pow(1 + r, months) - 1);
        setFinal(final.toFixed(2));
      } else
      {
        let final = Array(months).fill("a").map((item, i) =>
        {
          return ((amount / months) + (amount - i * +(amount / months).toFixed(2)) * r).toFixed(2);
        });
        setFinal(final);
      }
    } else
  ...
  function renderResult()
  {
    ...
    if (warning)
    {
      shouldBeRendered = <p><span className={styles.error}>{warning}</span></p>
    }
    else if (final)
    {
      shouldBeRendered = <div className={styles.paymentSchedule}>
        {
          Array.isArray(final) ? (
            final.map((item, i) => (
              <div key={i}>
    ...
    <div className={styles.calculator}>
      ...
      <form onSubmit={calculate}>
        ...
        <div>
          ...
          <input value={percent || ""} type="number" onChange={e => setPercent(+e.target.value)} />
        </div>
        <div>
          <label>Մարման ժամկետ:</label>
          <input value={months || ""} type="number" placeholder="ամիսներ" onChange={e => setMonths(+e.target.value)} />
        </div>
        <div>
